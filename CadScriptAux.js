...
#option-3:checked:checked ~ .option-3 span{
  ...
  async function downloadFrEdit(url) {
    ...
    firstPage.drawText('Turiaçu MA    ' + dataHj, {
      x: 230,
  ...
  const findPerson = async (mode = 1) => {
    ...
    await fetchData(url, data)
      .then(async () => {
        ...
        var validCad = document.querySelector('td[title="CADASTRADO"]').closest('tr').attributes[1]
        var infoFoundCad = validCad.value.split("'")
        var codFam = infoFoundCad[1];
        var digitoVerificador = infoFoundCad[3];
        validCad.value = ""
...
const addEscolaridadeShortcut = () => {
  ...
  const initEscCadCrawler = async (url) => {
    ...
    await frmSubmitMod(url).then(async () => {
      ...
      if (document.body.contains(document.getElementById('L_aviso'))) {
        ...
        await frmSubmitMod("carregarTelasPessoa.do?acao=iniciarAlterar&continuarAtualizacaoPessoa=true").then(async () => {
  ...
  document.getElementById('escolarIconBtn')
    .addEventListener('click', () => {
      initEscCadCrawler('carregarTelasPessoa.do?acao=confirmarSelecionarOutraPessoaParaEdicao&metodo=alterar');
    })
}
...
const addTrabalhoShortcut = () => {
  ...
  document.getElementById('trabalhoIconBtn')
    .addEventListener('click', () => {
      initTrabCadCrawler('carregarTelasPessoa.do?acao=confirmarSelecionarOutraPessoaParaEdicao&metodo=alterar');
    })
}
